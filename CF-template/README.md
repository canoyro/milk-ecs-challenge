...
docker tag milk-ecs-webapp:v2 <dockerhub-user>/milk-ecs-webapp:v2
docker push <dockerhub-user>/milk-ecs-webapp:v2
```
...
The workflow at `.github/workflows/deploy-webapp.yml` automates the image rollout:

- deploys or updates the CloudFormation stack
- reads the ECR repository URI from stack outputs
- builds `CF-template/app`
- pushes an image tagged with the first 12 characters of the Git commit SHA
- redeploys CloudFormation with `ContainerImage` set to that ECR image URI
